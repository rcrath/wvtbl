...
scrap_folder = os.path.join(base, 'scrap')
...
def normalize_audio_to_peak(file_path, target_peak=-6, cutoff=1.0):
    """
    Normalize the audio file to a target peak in dBFS and remove DC bias.

    Parameters:
    - file_path: Path to the audio file.
    - target_peak: Target peak level in dBFS.
    - cutoff: Cutoff frequency for the high-pass filter to remove DC bias.
    """
    data, samplerate = sf.read(file_path)
    
    # Remove DC bias with a high-pass filter
    data_filtered = highpass_filter(data, cutoff, samplerate)
    
    # Normalize
    peak = np.max(np.abs(data_filtered))
    if peak == 0:
        return  # Avoid division by zero if the audio is silent
    normalization_factor = 10 ** ((target_peak - 20 * np.log10(peak)) / 20)
    normalized_data = data_filtered * normalization_factor
...
if len(mapped_frequencies) > 0:
    ...
    print(f"Proceeding, detected frequency {round(mode_frequency)} Hz, with {round(mode_confidence_avg * 100)}% confidence.")
else:
...
if freq_est_manually_set:
    ...
    print(f"Detected frequency: {round(mode_frequency)} Hz with {round(mode_confidence_avg * 100)} confidence.%.")
    choice_prompt = f"Choose frequency source: \n1. Detected ({round(mode_frequency)} Hz), \n2. Manually entered ({freq_est} Hz) \n[default: 1]: "
    ...
    if user_choice == '2':
        ...
        print(f"Proceeding at: {mode_frequency} Hz with confidence {round(mode_confidence_avg * 100)}%.")
    ...
    print(f"Pitch detection is not reliable, {round(mode_frequency)} with only {round(mode_confidence_avg * 100)}%")
...
# Now print the counts before any processing
print(f"\b\033[A\033[KTotal segments before discarding: {total_segments_count}")
...
# After counting, proceed with discarding or processing files based on user choices
for file in os.listdir(singles_folder):
    if file.endswith(".wav"):
        # Construct the full path to the file
        file_path = os.path.join(singles_folder, file)

        # Discard or process the file based on the user's choices
        if '_atk' in file and discard_atk_choice == 'Y':
            os.remove(file_path)  # Discard attack file
            continue  # Skip to the next file
        elif '_dev' in file and discard_dev_choice == 'Y':
            os.remove(file_path)  # Discard deviant file
            continue  # Skip to the next file
        elif not ('_atk' in file or '_dev' in file) and discard_good_choice == 'Y':
            os.remove(file_path)  # Discard 'good' file
            continue  # Skip to the next file
...
if not os.path.exists(output_folder_for_adjusted_samples):
    os.makedirs(output_folder_for_adjusted_samples, exist_ok=True)
...
# Block 11 save serum wavtables

# Assuming {base}_{nearest_192_higher_pwr2}_all.wav and {base}_2048_all.wav are located in concat_folder
closest_pwr2_all_path = os.path.join(concat_folder, f"{base}_{nearest_192_higher_pwr2}_all.wav")
hz_94_all_path = os.path.join(concat_folder, f"{base}_2048_all.wav")
...
# Apply padding if needed right after reading the files
data_closest_pitch_padded = apply_padding_if_needed(data_closest_pitch, 524288, amplitude_tolerance_db)
data_94hz_padded = apply_padding_if_needed(data_94hz, 524288, amplitude_tolerance_db)

# Continue with your existing code logic, but use the padded data
# For example, when saving the padded data, you might adjust your function calls:
# Save padded data back to a temporary file before passing to split_and_save_wav_with_correct_padding
sf.write(output_path_pwr2, data_closest_pitch_padded, sr_closest_pitch)
sf.write(output_path_2048, data_94hz_padded, sr_94hz)

# Now proceed with your logic for creating wavetables
num_full_files_closest_pitch = math.ceil(len(data_closest_pitch_padded) / 524288)
split_and_save_wav_with_correct_padding(closest_pwr2_all_path, closest_pitch_folder, base, f"{nearest_192_higher_pwr2}", num_full_files_closest_pitch)

num_full_files_94hz = math.ceil(len(data_94hz_padded) / 524288)
split_and_save_wav_with_correct_padding(hz_94_all_path, hz_94_folder, base, "2048", num_full_files_94hz)

# The rest of your code for cleaning up and normalizing remains unchanged
stop_spinner = True
spinner_thread.join()
...
print(f"\b\b\033[A\033[KAll files in '{concat_folder}' have been normalized to peak -6 dBFS.")
