...
def process_hz_94_all_path(source_path, output_folder, base_name, wavetable_type, sr=192000):
    """
    Process and save segments from hz_94_all_path. The final segment is adjusted to be 524288 samples long.
    """
    data, _ = sf.read(source_path, dtype='float32')
    total_samples_per_file = 2048 * 256  # Fixed length for each wavetable segment

    num_segments = math.ceil(len(data) / total_samples_per_file)
    
    for i in range(num_segments):
        if i < num_segments - 1:
            # For all but the last segment, process as usual
            start_idx = i * total_samples_per_file
            segment_data = data[start_idx:start_idx + total_samples_per_file]
        else:
            # For the last segment, ensure it is exactly 524288 samples long
            # If the last segment is shorter, start it earlier in the data array to ensure it's full length
            last_segment_start = len(data) - total_samples_per_file
            segment_data = data[last_segment_start:]

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file_name = f"{base_name}_{wavetable_type}_{i+1}_{timestamp}.wav"
        output_file_path = os.path.join(output_folder, output_file_name)
        sf.write(output_file_path, segment_data, sr, subtype='FLOAT')
        print(f"Saved '{output_file_name}' with {len(segment_data)} samples.")

import soundfile as sf
import numpy as np

def process_closest_pwr2_all_path(input_path, output_folder, base_name, wavetable_type):
    # Read the input file, keeping its original sample rate
    data, original_sr = sf.read(input_path, dtype='float32')
    
    # Your processing logic here, including any padding as needed...
    # If padding is needed for files shorter than 524288 samples:
    if len(data) < 524288:
        padding_length = 524288 - len(data)
        padded_data = np.concatenate([data, np.zeros(padding_length, dtype=data.dtype)])
    else:
        padded_data = data  # Or any other logic for longer files
    
    # Construct output file name and path
    output_file_name = f"{base_name}_{wavetable_type}.wav"
    output_file_path = os.path.join(output_folder, output_file_name)

    # Write the output file with the original sample rate
    sf.write(output_file_path, padded_data, original_sr)
...
closest_pwr2_all_path = os.path.join(concat_folder, f"{base}_{nearest_192_higher_pwr2}_all{ext}")
...
# Assuming closest_pwr2_all_path and hz_94_all_path are paths to the full wavetable files

# 'closest_pitch' wavetable type
process_closest_pwr2_all_path(closest_pwr2_all_path, closest_pitch_folder, base, f"{nearest_192_higher_pwr2}")

# '94Hz' wavetable type
process_hz_94_all_path(hz_94_all_path, hz_94_folder, base, "_2048")
